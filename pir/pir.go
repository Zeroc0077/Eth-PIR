...
// PIR Defines the interface for PIR with preprocessing schemes
type PIR interface {
	Name() string
...
// RunFakePIR Run PIR's online phase, with a random preprocessing (to skip the offline phase).
...
func RunFakePIR(pi PIR, DB *Database, p Params, i []uint64,
	...
	printTime(start)
	online_comm := float64(query.Size() * p.Logq / (8.0 * 1024.0))
	...
	online_down := float64(answer.Size() * p.Logq / (8.0 * 1024.0))
...
// RunPIR Run full PIR scheme (offline + online phases).
func RunPIR(pi PIR, DB *Database, p Params, i []uint64) (float64, float64, bool, uint64) {
	fmt.Printf("Executing %s\n", pi.Name())
	//fmt.Printf("Memory limit: %d\n", debug.SetMemoryLimit(math.MaxInt64))
	debug.SetGCPercent(-1)
	...
	printTime(start)
	comm := float64(offline_download.Size() * p.Logq / (8.0 * 1024.0))
	...
	runtime.GC()
	printTime(start)
	comm = float64(query.Size() * p.Logq / (8.0 * 1024.0))
	fmt.Printf("\t\tOnline upload: %f KB\n", comm)
	bw += comm
	runtime.GC()
	...
	elapsed := printTime(start)
	rate := printRate(p, elapsed, len(i))
	comm = float64(answer.Size() * p.Logq / (8.0 * 1024.0))
	fmt.Printf("\t\tOnline download: %f KB\n", comm)
	bw += comm
	runtime.GC()
	...
	runtime.GC()
	debug.SetGCPercent(100)
	return rate, bw, true, val
}

// RunPIRCompressed Run full PIR scheme (offline + online phases), where the transmission of the A matrix is compressed.
