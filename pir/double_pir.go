...
// COMP_RATIO Ratio between first-level DB and second-level DB
...
func (pi *DoublePIR) FakeSetup(DB *Database, p Params) (State, float64) {
	...
	offline_download := float64(p.N*p.delta()*info.X*p.N*p.Logq) / (8.0 * 1024.0)
	fmt.Printf("\t\tOffline download: %d KB\n", uint64(offline_download))
...
func (pi *DoublePIR) Answer(DB *Database, query MsgSlice, server State, _ State, p Params) Msg {
	H1 := server.Data[0]
